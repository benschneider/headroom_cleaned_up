...
Managed RTK installs include a Headroom shim that keeps source inspection exact:
`rtk read`, `rtk cat`, `rtk sed`, `rtk nl`, `rtk diff`, `rtk git diff`, and
`rtk git show` bypass compression. Use `rtk --raw <command> ...` for any other
command where exact output matters.
