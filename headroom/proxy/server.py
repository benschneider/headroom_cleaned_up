...
class _DisabledTelemetry:
    ...
    def get_tool_stats(self, signature_hash: str) -> dict[str, Any] | None:
        _ = signature_hash
        return None

    def get_recommendations(self, signature_hash: str) -> dict[str, Any]:
        _ = signature_hash
        return {}
...
def get_telemetry_collector() -> _DisabledTelemetry:
    return _DisabledTelemetry()


def get_toin() -> _DisabledToin:
    return _DisabledToin()
...
def _apply_stateless_persistence(config: ProxyConfig) -> None:
    ...
    if not getattr(config, "stateless", False):
        return
    # The cleaned-up fork has no TOIN persistence package. Keep this as a
    # no-op so restored proxy startup remains compatible with stateless mode.
